import { EditIcon, QuillIcon, SearchIcon, XIcon } from "@/components/icons";
import { Fonts, rgba, TrenaColorPalette } from "@/constants/theme";
...
import {
  ...
  getAddExerciseDraft,
  setAddExerciseDraft,
} from "@/lib/workouts/methods/ui-draft";
import {
  consumeMethodInstanceCreatedQueue,
  subscribeMethodInstanceCreated,
} from "@/lib/workouts/methods/ui-events";
import {
...
function inferExerciseFromMethod(args: {
  ...
  const lift =
    args.binding.methodKey === "wendler_531" ? args.binding.lift : "bench";
  if (lift === "squat")
    return { kind: "learn", learnExerciseId: "exercise-squat" };
  if (lift === "bench")
    return { kind: "learn", learnExerciseId: "exercise-bench" };
  if (lift === "deadlift")
    return { kind: "learn", learnExerciseId: "exercise-deadlift" };
  return { kind: "free", name: "Overhead Press" };
}

function matchesAmrapExercise(mi: MethodInstanceRow, ex: ExerciseRef) {
  if (mi.method_key !== "amrap" && (mi.method_key as any) !== "bilbo")
    return false;
  ...
  if (cfgEx.kind === "learn" && ex.kind === "learn") {
    return cfgEx.learnExerciseId === ex.learnExerciseId;
  }
  return false;
}
...
export function AddExerciseModal({
  ...
  const [selectedSelections, setSelectedSelections] = React.useState<
    AddExerciseSelection[]
  >([]);

  const [methodChoice, setMethodChoice] = React.useState<MethodKey | null>(
    null,
  );
  const [selectedMethodInstanceId, setSelectedMethodInstanceId] =
    React.useState<string | null>(null);
  const [wendlerLift, setWendlerLift] = React.useState<WendlerLiftKey>("bench");

  const [awaitingCreatedMethodKey, setAwaitingCreatedMethodKey] =
    React.useState<MethodKey | null>(null);
  ...
  React.useEffect(() => {
    ...
    setSelectedSelections(
      d?.selectedExercise
        ? [{ exercise: d.selectedExercise, method: null }]
        : [],
    );
  ...
  React.useEffect(() => {
    ...
    const unsub = subscribeMethodInstanceCreated((row) => {
      ...
      if (
        awaitingCreatedMethodKey &&
        row.method_key !== awaitingCreatedMethodKey
      )
        return;
      ...
      const binding: MethodBinding =
        row.method_key === "amrap"
          ? { methodKey: "amrap" }
          : { methodKey: "wendler_531", lift: wendlerLift };
      ...
      if (ex) {
        const sel: AddExerciseSelection = {
          exercise: ex,
          method: { methodInstanceId: row.id, binding, methodInstance: row },
        };
  ...
  React.useEffect(() => {
    ...
    const row = queued[0];
    if (awaitingCreatedMethodKey && row.method_key !== awaitingCreatedMethodKey)
      return;
    setMethods((cur) => [row, ...cur.filter((x) => x.id !== row.id)]);
    setMethodChoice(row.method_key);
    setSelectedMethodInstanceId(row.id);
  ...
  const exactMatch = React.useMemo(() => {
    const t = normalize(term);
    return (
      filteredLearn.some((x) => normalize(x.name) === t) ||
      filteredCustom.some((x) => normalize(x) === t)
    );
  }, [filteredCustom, filteredLearn, term]);

  const amraps = React.useMemo(
    () =>
      methods.filter(
        (m) => m.method_key === "amrap" || (m.method_key as any) === "bilbo",
      ),
    [methods],
  );
  const wendlers = React.useMemo(
    () => methods.filter((m) => m.method_key === "wendler_531"),
    [methods],
  );
  ...
  const suggestedAmraps = React.useMemo(() => {
    ...
    const lastEx =
      selectedSelections.length > 0
        ? selectedSelections[selectedSelections.length - 1].exercise
        : null;
  ...
  const selectedMethodInstance = React.useMemo(
    () =>
      selectedMethodInstanceId
        ? (methods.find((m) => m.id === selectedMethodInstanceId) ?? null)
        : null,
  ...
  const isSelected = (ref: ExerciseRef) => {
    return selectedSelections.some((s) => {
      ...
      if (s.exercise.kind === "free")
        return s.exercise.name === (ref as any).name;
      if (s.exercise.kind === "learn" && ref.kind === "learn") {
        return s.exercise.learnExerciseId === ref.learnExerciseId;
      }
      return false;
    });
  };
  ...
  const isMethodSelected = (miId: string, binding?: MethodBinding) => {
    return selectedSelections.some((s) => {
      ...
      if (
        s.method.binding.methodKey === "wendler_531" &&
        (s.method.binding as any).lift !== (binding as any).lift
      )
        return false;
      return true;
    });
  ...
  const onSelectMethod = (mi: MethodInstanceRow) => {
    ...
    setSelectedSelections((cur) => {
      ...
      return [
        ...
        {
          exercise: ex,
          method: { methodInstanceId: mi.id, binding, methodInstance: mi },
        },
      ];
    });
  };
  ...
  const onEditMethod = (methodInstanceId: string) => {
    ...
    onClose();
    router.push(
      `/activities/programs?editId=${encodeURIComponent(methodInstanceId)}&returnTo=selector` as any,
    );
  };
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.searchRow}>
          ...
          {term.length > 0 && (
            <Pressable
              ...
              style={{ padding: 4 }}
            >
              <XIcon size={16} color={rgba(colors.text, 0.5)} />
            </Pressable>
          )}
        </View>
        ...
        >
          <View style={styles.section}>
            <Text style={[styles.sectionHeader, styles.sectionHeaderTop]}>
              Progression
            </Text>
                ...
                <View style={{ gap: 10 }}>
                  <View style={styles.rowBetween}>
                    <Text style={styles.muted}>
                      Select an AMRAP progression
                    </Text>
                    <Pressable
                      accessibilityRole="button"
                      onPress={() => onCreateMethod("amrap")}
              ...
              {methodChoice === "wendler_531" ? (
                <View style={{ gap: 10 }}>
                  <View style={styles.rowBetween}>
                    ...
                    <Pressable
                      accessibilityRole="button"
                      onPress={() => onCreateMethod("wendler_531")}
                      style={styles.linkButton}
                    >
                      <Text style={styles.linkText}>Create new</Text>
                    </Pressable>
                  ...
                  {selectedMethodInstanceId ? (
                        ...
                        {(
                          ...
                          return (
                            <Pill
                              ...
                              onPress={() => {
                                setWendlerLift(k);
                                const mi = wendlers.find(
                                  (m) => m.id === selectedMethodInstanceId,
                                );
                    ...
                    <Text style={styles.muted}>
                      Pick a progression first, then choose which lift to log.
                    </Text>
                  )}
                </View>
              ) : null}
            ...
            {term && !exactMatch ? (
              <Pressable
                ...
                onPress={() => {
                  ...
                  haptics.success(); // Feedback for creation
                  setCustoms((prev) => [name, ...prev]);
                  // Do NOT clear term. This allows the user to see the newly created exercise
                  // in the list immediately (as it now matches 'exactMatch').
                  // setTerm('');
                  onSelectExercise({ kind: "free", name });
                }}
              >
                <View
                    ...
                    >
                      ...
                      >
                        <QuillIcon
                          size={16}
                          color={
                            selected ? colors.onPrimary : rgba(colors.text, 0.5)
                          }
                        />
                      </View>
                      <Text
                    ...
                    >
                      ...
                      >
                        <Text
                          style={[
                            styles.iconText,
                            selected && styles.iconTextSelected,
                          ]}
                        >
                          {ex.name.slice(0, 1)}
                        </Text>
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        <Text
                          style={[
                            styles.rowMeta,
                            selected && styles.rowMetaSelected,
                          ]}
                        >
                          {ex.equipment.join(", ")}
                        </Text>
...
const createStyles = (colors: TrenaColorPalette) =>
  StyleSheet.create({
    ...
    divider: { height: 1, backgroundColor: rgba(colors.text, 0.1) },
    empty: {
      textAlign: "center",
      color: rgba(colors.text, 0.4),
      marginTop: 10,
    ...
    footer: {
      ...
      paddingVertical: 12,
      borderTopWidth: 1,
      borderTopColor: rgba(colors.text, 0.1),
      flexDirection: "row",
      alignItems: "center",
      gap: 12,
    },
